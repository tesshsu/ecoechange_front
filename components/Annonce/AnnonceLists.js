	...
	useEffect(() => {
		...
		setFilteredIdeas(
			ideas?.filter((idea) =>
				...
				idea?.sub_category?.toLowerCase().includes(search.toLowerCase())
			)
		);
	}, [search, ideas]);
	...
	return (
		<>
			...
			{filteredIdeas?.map((idea, idx) => idea?.premium ?
				(
					<div key={idx} id={idea.id} className="preniumAnnonce relative w-full md:w-6/12 lg:w-4/12 lg:mb-0 mb-12 mr-4 my-6 shadow-lg max-w-400-px rounded-lg border-2 border-gray-200 ">
						<img
							alt="..."
					...
					<div key={idx} id={idea.id} className="classicAnnonce relative w-full md:w-6/12 lg:w-4/12 lg:mb-0 mb-12 mr-4 my-6 shadow-lg max-w-400-px rounded-lg border-2 border-gray-200 ">
						{(() => {
							...
							}else if(idea?.note >= 9 && idea?.votes?.confirmed == 5) {
								return (
									<img
										alt="..."
										src={require("assets/img/note/certificate/certified.png")}
										className="w-full align-left certifiImage animate-bounce-small"
									/>
								)
							} else {
								return (
							...
							<Link href={idea?.id ? (`/annonce?id=${idea?.id}`) : ("#")} {...idea}>
								...
								>
									<div className="descriptionAnnonce">
										<span className="uppercase text-lg">{idea?.category} - {idea.sub_category}</span>
										{isFavorite(idea?.id) ? (
											<i className="fas fa-heart text-green-500 text-2xl ml-2"> </i>
										): (<FavorisButton category="idea" entity_id={idea?.id} action={onClickFavoris}/>)}
										...
										<p className="text-md text-green-500 truncate animate-bounce">{idea?.title}</p>
									</div>
									<p className="text-gray-700 text-md">
										Idée Pour :  {renderSwitchValue(idea?.usage)}
